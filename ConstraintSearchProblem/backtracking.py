...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    # if trace: print("==>Trying {}".format(nxtvar.name()))
    #
    # "Assign every value to the next variable from its domain"
    # for val in nxtvar.domain():
    #     if trace: print("==> {} = {}".format(nxtvar.name(), val))
    #
    #     nxtvar.setValue(val)
    ...
    for val in nextVar.curDomain():
        ...
        for cnstr in csp.constraintsOf(nextVar):
            if cnstr.numUnassigned() == 1:
                if FCCheck(cnstr, nextVar, val) == "DWO":
        ...
        if noDWO:
            ...
            if len(solns) > 0 and not allSolutions:
                Variable.restoreValues(nextVar, val)
                # don't bother with other values of nxtvar as we found a soln.
                break
