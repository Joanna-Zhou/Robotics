...
class QueensTableConstraint(TableConstraint):
    '''Queens constraint between queen in row i and row j, but
       ...
       call TableConstraint.__init__(self, name, scope, satisfyingAssignments)
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        diagnolcheck = abs(i - j)
        for column_i in qi.domain():
            for column_j in qj.domain():
                # Not same column and not diagnol
                if column_i != column_j and abs(column_i - column_j) != diagnolcheck:
