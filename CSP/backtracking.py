...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    "Can all the constraints be satisfied? If no, return DWO, if yes, return OK."
    "Satisfying a constraint means that every variable in its scope can be assigned with any value in its domain"
    "If one of the variables has its domain revised, other constraints of that variable (i.e. others who need to be rechecked) are enqueued."
    while len(constraints) > 0:
        cnstr = constraints.pop()
        for nxtvar in cnstr.scope():
            for val in nxtvar.curDomain():
                "If you can't find an assignment for all other variables in the scope with constraints satisfied"
                if not cnstr.hasSupport(nxtvar, val):
                    nxtvar.pruneValue(val, reasonVar, reasonVal)
                    "If already no other possible values left, domain wiped out, return immediately"
                    if nxtvar.curDomainSize() == 0:
                        return "DWO"
                    "Else, just push the revised variable back to be checked again"
                    for modifiedcnstr in csp.constraintsOf(nxtvar):
                        if modifiedcnstr != cnstr and not modifiedcnstr in constraints:
                            constraints.append(modifiedcnstr)
    return "OK"
